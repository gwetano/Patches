...
const win32_displayconfig_1 = require("@seorii/win32-displayconfig");
...
function isInSnapZone() {
    ...
    return ((point.x > display.bounds.x - 20 && point.x < display.bounds.x + 20) ||
        (point.x > display.bounds.x + display.bounds.width - 20 &&
            point.x < display.bounds.x + display.bounds.width + 20));
}
/**
 ...
 */
function win10refresh(win, maximumRefreshRate) {
    ...
    win.on('will-move', (e, newBounds) => {
        ...
        if (lastWillMoveBounds !== undefined &&
            areBoundsEqual(lastWillMoveBounds, newBounds)) {
            e.preventDefault();
            return;
        }
        ...
        const didOptimisticMove = !isMoving &&
            guardingAgainstMoveUpdate(() => {
                    ...
                    const didIt = guardingAgainstMoveUpdate(() => {
                        ...
                        if (lastWillResizeBounds && lastWillResizeBounds.width)
                            setWindowBounds({
                                ...
                                width: Math.floor(lastWillResizeBounds.width /
                                    electron.screen.getDisplayMatching(basisBounds).scaleFactor),
                                height: Math.floor(lastWillResizeBounds.height /
                                    electron.screen.getDisplayMatching(basisBounds).scaleFactor),
                            });
                        else
                            setWindowBounds({
                                x: Math.floor(basisBounds.x + (cursor.x - basisCursor.x)),
                                y: Math.floor(basisBounds.y + (cursor.y - basisCursor.y)),
                                width: Math.floor(lastWillMoveBounds.width /
                                    electron.screen.getDisplayMatching(basisBounds).scaleFactor),
                                height: Math.floor(lastWillMoveBounds.height /
                                    electron.screen.getDisplayMatching(basisBounds).scaleFactor),
                            });
                    });
                    if (didIt) {
    ...
    win.on('will-resize', (e, newBounds) => {
        if (lastWillResizeBounds !== undefined &&
            areBoundsEqual(lastWillResizeBounds, newBounds)) {
            e.preventDefault();
            return;
        }
...
exports.default = win10refresh;
//# sourceMappingURL=win10refresh.js.map